...
| Card | Source | Colour |
|------|--------|--------|
| To Mark | Count of `teacherReviewStatus === "to_mark"` across live assessments | Red (#c24e3f) |
| Due Today | Assessments with `dueDate` matching today | Blue (#2563eb) |
| Ready to Release | Grades with `gradingStatus === "ready"` and no `releasedAt` | Green (#16a34a) |
| Reports Due | Unpublished report drafts in open/closing cycles | Amber (#b45309) |

### Interactive Timetable Sidebar

- Vertical time rail showing today's schedule period-by-period
- Includes breaks and lunch rows (matching the structured time-slot pattern from `deriveTimeRows()`)
...
- Below the timetable, stacked mini reference panels:
  ...
  - **Recent Activity**: Last 3 items (new artifacts, announcement replies, incidents)
...
- `generateRecommendations(state, now, classFilter?)` — master function, returns sorted `Recommendation[]`
...
- **Scan**: Each live assessment → count students where `teacherReviewStatus === "to_mark"`
...
- **Item (distribution)**: "Distribute N published reports — {cycleName}" (for reports published but not distributed)
- **Deadline**: Cycle `endDate`
- **Deep-link**: `/reports/cycles/{id}`

#### 3. Student Attention Flags (weight: 4)

- **Scan**: `computeAttentionStudents()` — students averaging below 50% or beginning mastery
...
| Lesson prep | Lesson plan assigned to next upcoming period | "Review lesson plan — {lessonTitle} ({className})" |
...
- No changes to other portals (student, family, admin)
...
- `useEffect` syncs `recommendationClassFilter` to store's `activeClassId` when macro filter changes
- Small toggle in recommendation section header allows override back to "All"
- Engine receives the filter and scopes output accordingly
